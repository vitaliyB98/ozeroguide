<?php
	class Model_News extends Model {
		...
		public function setViews ($id) {
			if ( !isset( $_COOKIE[$id] )) {
				$DBH = DB::Connect();
				$STH = $DBH->query("SELECT `views` FROM `news` WHERE id = $id");
				$STH->setFetchMode(PDO::FETCH_ASSOC); 

				$row = $STH->fetch();
				$views = $row['views']+1;

				$STH = $DBH->prepare("UPDATE `news` SET `views` = $views WHERE id = $id");  
				$STH->execute();
		...
		public function save ($data,$files) {	
			...
			$DBH = DB::Connect();
			$STH = $DBH->prepare("INSERT INTO news ( `title`, `content`, `photo`, `date`, `user_id` ) values ( '$this->title', '$this->content', '$this->photo', '$this->date', '$this->userId')");  
			$STH->execute();
		}
		...
		public function update ($data, $files, $id) {
			...
			$DBH = DB::Connect();
			$STH = $DBH->prepare("UPDATE `news` SET `title` = '{$data['title']}', `content` = '{$data['content']}' $add_photo WHERE id = $id");  
			$STH->execute();
		}
